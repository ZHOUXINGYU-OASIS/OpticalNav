function [ErrData, TimeData, stdData, rmseData, time_cost, MDData] = MC_OD_LOS_VBR(case_id)
...
for i_MC = 1: Num
    ...
    for i = 1: number + 1
        ...
        if i <= len + 0 % doesn't reverse in the first 10 measurements
            ...
            xai = statea(1,:)';
            P0i = P0;
            H0 = cal_H_angle_dis(xsei, xai);
            H0 = H0(end,:);
            % Assume that the ephemeris of the asteroid is precise
            [dy, H, R, y, ratio, ratio_fit, Rd] = ...
                data_smooth(xs, xa, ...
            ...
            H0 = H0(end,:);
            % Assume that the ephemeris of the asteroid is precise
            [dy, H, R, y, ratio, ratio_fit, Rd, de, d0e_] = ...
                data_smooth(xs, xa, ...
...
meanData = zeros(number + 1, ns * 2);
for i = 1: number + 1
    for j = 1: ns * 2
        meanData(i,j) = mean(ErrData(:,j,i));
    end
end
